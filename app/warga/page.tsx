    ...
    <div className="min-h-screen bg-[#0F1220] text-white p-4 lg:p-6 pb-24">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="card rounded-[28px] p-7 glow mb-6">
          ...
          <div className="flex gap-2 mt-4"><button onClick={()=>setTab('iuran')} className={`rounded-full px-4 py-1.5 text-[11px] ${tab==='iuran'?'bg-white text-black font-bold':'bg-[#252A42] text-slate-400'}`}>Iuran Saya ({iuran.length}) - BELUM: {iuran.filter((x:any)=>x.status==='BELUM').length}</button></div>
        </div>
        <div className="card rounded-[24px] p-6">
          <h3 className="font-bold mb-4">Tagihan Saya - {wargaData?.nama}</h3>
          <div className="space-y-2">
            {iuran.length===0 ? <div className="bg-[#0F1220] border border-dashed border-[#2A2F4A] rounded-2xl p-8 text-center text-[12px] text-slate-500">Belum ada tagihan</div> :
              iuran.map((i:any)=>(
                <div key={i.id} className="flex justify-between items-center bg-[#0F1220] border border-[#2A2F4A]/50 rounded-full px-5 py-3 text-[12px]">
                  <div><div className="font-medium">Bulan {i.bulan}/{i.tahun} - Rp {i.nominal?.toLocaleString()}</div><div className="text-[10px] text-slate-500">{i.jenis} • {i.status} • {i.metode||'-'}</div></div>
      ...
      {showQRIS && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="card rounded-[24px] p-6 max-w-sm w-full text-center">
            <h3 className="font-bold mb-2">Bayar QRIS - Bulan {showQRIS.bulan}/{showQRIS.tahun}</h3>
