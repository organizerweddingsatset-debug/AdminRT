...
-- 6. Enable RLS dan buat policy public read/write untuk MVP (nanti bisa diperketat)
...
drop policy if exists "public all warga" on warga;
drop policy if exists "public all iuran" on iuran;
drop policy if exists "public all pengeluaran" on pengeluaran;
drop policy if exists "public all pengumuman" on pengumuman;
drop policy if exists "public all inventaris" on inventaris;
...
-- Seed data awal (3 warga + iuran contoh)
...
on conflict (nik, bulan, tahun) do nothing;
