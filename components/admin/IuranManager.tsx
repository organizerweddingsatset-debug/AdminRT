    ...
    <div className="space-y-4">
      <div className="card rounded-[24px] p-6">
        ...
        <div className="grid grid-cols-4 gap-3">
          <div><label className="text-[10px] text-slate-500">Nominal / bulan</label><input type="number" value={config.nominal} onChange={e=>setConfig({...config,nominal:parseInt(e.target.value)||0})} className="input mt-1" /></div>
          <div><label className="text-[10px] text-slate-500">Tahun Berlaku</label><input type="number" value={config.tahun} onChange={e=>setConfig({...config,tahun:parseInt(e.target.value)||2026})} className="input mt-1" /></div>
          <div><label className="text-[10px] text-slate-500">QRIS URL / Image</label><input value={qrisUrl} onChange={e=>setQrisUrl(e.target.value)} placeholder="https://.../qris.png" className="input mt-1" /></div>
          <div className="flex items-end gap-2"><button onClick={saveConfig} className="bg-violet-600 text-white rounded-full px-5 py-2.5 text-[12px] font-bold w-full">Simpan Config</button></div>
        </div>
        <button onClick={generateTagihan} className="mt-4 w-full bg-white text-black rounded-full py-3 text-[13px] font-bold">⚡ Generate Tagihan Bulan Ini ke {warga.length} Warga (Belum Lunas)</button>
      </div>
      ...
      {showQRIS && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="card rounded-[24px] p-6 max-w-sm w-full text-center">
            ...
            {qrisUrl ? <img src={qrisUrl} className="w-full rounded-xl mb-3" alt="QRIS" /> : <div className="bg-[#0F1220] border border-dashed border-[#2A2F4A] rounded-xl p-10 text-[11px] text-slate-500 mb-3">QRIS belum di-set di Pengaturan</div>}
