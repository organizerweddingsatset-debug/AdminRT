      ...
      <div className="card rounded-[24px] p-6">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex gap-2">
            <button onClick={()=>setFilter('belum')} className={`rounded-full px-4 py-1.5 text-[11px] ${filter==='belum'?'bg-white text-black font-bold':'bg-[#252A42] text-slate-400'}`}>Belum Lunas ({iuran.filter(i=>(i.status||'').toUpperCase()!=='LUNAS').length})</button>
        ...
        <div className="space-y-2 max-h-[600px] overflow-auto">
          {filtered.map((i:any)=>(
            <div key={i.id} className="flex justify-between items-center bg-[#0F1220] border border-[#2A2F4A]/50 rounded-full px-5 py-3">
              ...
              <div className="flex gap-2">
                {(i.status||'').toUpperCase()!=='LUNAS' ? <>
                  <button onClick={()=>setShowQRIS(i)} className="bg-[#1C2035] border border-[#2A2F4A] rounded-full px-3 py-1.5 text-[11px]">💳 QRIS</button>
                  <button onClick={()=>bayarCash(i.id)} className="bg-white text-black rounded-full px-3 py-1.5 text-[11px] font-bold">Cash</button>
                </> : <button onClick={()=>cetakKuitansi(i)} className="bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 rounded-full px-4 py-1.5 text-[11px] font-bold">🧾 Kuitansi Sultan</button>}
              </div>
            </div>
          ))}
        </div>
