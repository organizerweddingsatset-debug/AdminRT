...
export default function IuranManager({warga}:{warga:any[]}){
  ...
  const load=async()=>{
    // FIX: tidak pakai join warga(nama) lagi, ambil manual biar nggak gagal kalau FK belum ada
    const { data, error } = await supabase.from('iuran').select('*').order('created_at',{ascending:false})
    if(error) console.error('Load iuran error', error)
    // merge dengan data warga dari prop
    const merged = (data||[]).map((i:any)=>{
      const w = warga.find((x:any)=>x.id===i.warga_id)
      return {...i, warga: w || {nama: 'Warga tidak ditemukan', alamat: i.warga_id?.slice(0,8)}}
    })
    setIuran(merged)
  ...
  const generateTagihan=async()=>{
    ...
    if(!confirm(`Generate tagihan ${bulan} ${tahun} Rp ${config.nominal} untuk ${warga.length} warga?`)) return
    let count=0
    for(const w of warga){
      const { data: exist } = await supabase.from('iuran').select('id').eq('warga_id',w.id).eq('bulan',bulan).eq('tahun',tahun).eq('jenis','wajib').maybeSingle()
      if(!exist){
        const { error } = await supabase.from('iuran').insert([{warga_id:w.id,bulan,tahun,nominal:config.nominal,status:'belum_lunas',jenis:'wajib'}])
        if(!error) count++; else console.error(error)
      }
    }
    alert(`Berhasil generate ${count} tagihan baru (dari ${warga.length} warga)`)
    load()
  }
  ...
  return (
    <div className="space-y-4">
      <div className="card rounded-[24px] p-6">
        <h3 className="font-bold mb-2">Iuran Wajib - {warga.length} warga</h3>
        <p className="text-[11px] text-slate-500 mb-4">Generate akan buat tagihan untuk semua warga di tabel Data Warga, termasuk yang baru ditambah</p>
      ...
      <div className="card rounded-[24px] p-6">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex gap-2"><button onClick={()=>setFilter('belum')} className={`rounded-full px-4 py-1.5 text-[11px] ${filter==='belum'?'bg-white text-black font-bold':'bg-[#252A42] text-slate-400'}`}>Belum Lunas ({iuran.filter(i=>i.status!=='lunas').length})</button><button onClick={()=>setFilter('lunas')} className={`rounded-full px-4 py-1.5 text-[11px] ${filter==='lunas'?'bg-white text-black font-bold':'bg-[#252A42] text-slate-400'}`}>Lunas</button><button onClick={()=>setFilter('all')} className={`rounded-full px-4 py-1.5 text-[11px] ${filter==='all'?'bg-white text-black font-bold':'bg-[#252A42] text-slate-400'}`}>Semua</button></div>
        </div>
        <div className="space-y-2 max-h-[600px] overflow-auto">
          {filtered.length===0 ? <div className="text-center py-10 text-slate-500 text-[12px]">Tidak ada tagihan {filter}. Klik Generate Tagihan di atas.</div> :
          filtered.map((i:any)=>(
            <div key={i.id} className="flex justify-between items-center bg-[#0F1220] border border-[#2A2F4A]/50 rounded-full px-5 py-3">
              ...
              <div className="flex gap-2">{i.status!=='lunas' ? <><button onClick={()=>setShowQRIS(i)} className="bg-[#1C2035] border border-[#2A2F4A] rounded-full px-3 py-1.5 text-[11px]">💳 QRIS</button><button onClick={()=>bayarCash(i.id)} className="bg-white text-black rounded-full px-3 py-1.5 text-[11px] font-bold">Cash</button></> : <button onClick={()=>cetakKuitansi(i)} className="bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 rounded-full px-4 py-1.5 text-[11px] font-bold">Kuitansi</button>}</div>
            </div>
          ))}
        </div>
      </div>
